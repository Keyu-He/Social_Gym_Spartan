...
class WerewolfActionHandler(ActionHandler):
    ...
    def handle_action(
        ...
        if env.current_state == "Day_vote":
            ...
            # Witch uses potions (each available at most once across the whole game).
            role = env.agent_to_role.get(agent_name, "")
            if role == "Witch" and action.action_type == "action":
                if "save" in action.argument.lower() and env.internal_state.get(
                    "witch_have_save", False
                ):
                    words = action.argument.split()
                    target = next(
                        (
                ...
                elif "poison" in action.argument.lower() and env.internal_state.get(
                    "witch_have_poison", False
                ):
                    words = action.argument.split()
                    target = next(
                        (
...
def create_agents(
    ...
    for idx, profile in enumerate(agent_profiles):
        ...
        # Check if agent is a werewolf (compare against the actual goal text, since
        # agent_goals[idx] stores the full goal string -- not the role label).
        is_werewolf = agent_name in werewolves
